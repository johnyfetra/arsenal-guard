...
export const valueProps = [
	{ icon: 'FileSearch', title: 'Un rapport clair, pas un dump de tickets', text: 'Chaque bug est documenté, priorisé et illustré. Pas de bruit — juste ce qui bloque vraiment.' },
	{ icon: 'Zap', title: 'Aucun setup, aucun outil à apprendre', text: 'Vous partagez un accès staging. On s\'occupe du reste. Livraison sous 48 à 72h ouvrées.' },
	{ icon: 'Eye', title: 'Le regard tiers que votre équipe ne peut pas avoir', text: 'Un testeur externe voit ce que la familiarité masque : parcours brisés, incohérences UX, anomalies critiques.' },
];
...
export const services: Service[] = [
	{
		...
		description: "Un testeur humain parcourt vos pages et documente chaque anomalie — sans prérequis technique, livré sous 48 à 72 heures.",
		...
		faq: [
			["Quelle différence avec l'Express QA Audit ?", "L'Express QA Audit est un diagnostic stratégique : il cartographie votre couverture de test et priorise ce qu'il faut automatiser. Le QA Snapshot est une exécution opérationnelle : un testeur humain parcourt vos pages et documente les anomalies réelles. Les deux sont complémentaires."],
			["Avez-vous besoin d'accès au code source ?", "Non. Un accès à votre environnement de test (staging, préprod ou production avec compte dédié) suffit. Le code source n'est jamais requis."],
			["Que se passe-t-il si le périmètre dépasse le tier choisi ?", "Un devis d'avenant est soumis avant de continuer. Aucune heure supplémentaire n'est facturée sans votre validation explicite."],
			["Les bugs sont-ils créés directement dans notre outil (Jira, Linear…) ?", "Par défaut, le rapport est livré en PDF. Sur demande, les bugs peuvent être créés dans votre outil de ticketing (Jira, Linear, GitHub Issues) — inclus sans surcoût pour les tiers Standard et Complet."],
			["Ce service couvre-t-il les applications mobiles ?", "Oui, via émulateur navigateur (responsive) ou sur device physique si vous fournissez un accès. Les tiers restent basés sur le nombre d'écrans testés."],
		],
		tiers: [
...
export type PunctualRow =
	| [string, string, string]
	| { name: string; duration: string; tiers: [string, string][] }
...
export const faq = [
	["Mon produit est déjà en production. Le QA Snapshot est-il utile pour moi ?", "Absolument. Le QA Snapshot est conçu pour tous les produits en ligne — pas seulement pour les lancements. Audit d'un site existant, validation avant une mise à jour critique, regard tiers sur un parcours client : toutes ces situations sont couvertes, quelle que soit la maturité de votre produit."],
	["Je suis développeur ou chef de projet en agence. Puis-je utiliser le Snapshot pour mes livrables clients ?", "Oui. Beaucoup de nos clients sont des agences ou des freelances qui veulent livrer avec un rapport de bugs indépendant avant la réception. Le QA Snapshot devient votre assurance qualité externe, sans temps interne à y consacrer."],
